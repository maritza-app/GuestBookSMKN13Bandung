...
<body>
    ...
    <div class="layout-container">
        ...
        <main class="main-content">
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
                <center><h2 class="m-0" style="color: #a83232">Data Kunjungan Umum</h2></center>
                <a href="{{ route('umum.create') }}" class="btn-add mb-3">+ Tambah Data</a>
            </div>
            <br>
            ...
            <thead class="table-dark">
                <tr>
                    ...
                    <th>Alamat</th>
                    <th>No Telepon</th>
                    <th>Tujuan Guru / TU</th>
                    <th>Tujuan Kepada Siswa</th>
                    <th>Keperluan</th>
                    <th>Foto</th>
                    <th>Aksi</th>
                </tr>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $row->id_kunjungan_umum }}</td>
                    <td>{{ $row->tanggal_kunjungan }}</td>
                    <td>{{ $row->nama_pengunjung_umum }}</td>
                    <td>{{ $row->berkunjung_sebagai }}</td>
                    <td>{{ $row->alamat }}</td>
    ...
    @else
        <span class="text-muted">Tidak berkunjung kepada guru / TU</span>
    @endif
</td>
                    <td>
    @if($row->siswa)
    ...
    @else
        <span class="text-muted">Tidak berkunjung kepada siswa</span>
    @endif
</td>
                    <td>{{ $row->keperluan }}</td>
                    <td>
                        <img src="{{ asset('imgumum/' . $row->foto) }}" alt="Foto" class="img-thumbnail" width="100">
                    </td>
                    <td>
                        <a href="{{ route('umum.edit', $row->id_kunjungan_umum) }}" class="btn-edit" style="margin-bottom: 10px;">🖊️ Edit</a>
                        <form action="{{ route('umum.destroy', $row->id_kunjungan_umum) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
