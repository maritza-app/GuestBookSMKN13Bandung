 ...
 <nav class="navbar">
    ...
    <div class="container-fluid mt-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
           <center><h2 class="m-0" style="color: #a83232">Data Kunjungan Instansi</h2></center>
            <a href="{{ route('instansi.create') }}"  class="btn-add mb-3">+ Tambah Data</a>
        </div>
        <br>
      ...
      <table id="slideTable" class="table table-striped table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $row->id_kunjungan_instansi }}</td>
                    <td>{{ $row->tanggal_kunjungan_instansi }}</td>
                    <td>{{ $row->nama_pengunjung }}</td>
                    <td>{{ $row->nama_instansi }}</td>
                    <td>{{ $row->alamat_instansi }}</td>
                    <td>{{ $row->telepon_instansi_pengunjung }}</td>
                    <td>
    @if($row->guru_tu)
        {{ $row->guru_tu->nip }} - {{ $row->guru_tu->nama_guru_tu }}
    @else
        <span class="text-muted">Data guru sudah terhapus</span>
    @endif
</td>
                    <td>{{ $row->keperluan }}</td>
                  
                    <td>
                        <img src="{{ asset('imginstansi/' . $row->foto) }}" alt="Foto" class="img-thumbnail" width="100">
                    </td>
                    <td>
                        <a href="{{ route('instansi.edit', $row->id_kunjungan_instansi) }}"class="btn-edit" style="margin-bottom: 10px;">🖊️ Edit</a>
                        <form action="{{ route('instansi.destroy', $row->id_kunjungan_instansi) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
