   ...
   <nav class="navbar">
    ...
    <div class="layout-container">
        ...
        <main class="main-content">
            <div class="container">
                <h1>Form Tambah Data Ortu/Wali</h1>
                
                <form action="{{ route('ortu_wali.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                        
                    <div class="mb-3">
                        <label for="tanggal_kunjungan_ow">Tanggal Kunjungan</label>
                        <input type="datetime-local" name="tanggal_kunjungan_ow" id="tanggal_kunjungan_ow" readonly>
                    </div>
                        ...
                        <select name="id_siswa" id="id_siswa" class="form-select" required>
                            ...
                            @foreach($siswa as $item)
                                <option 
                                    ...
                                    data-nama-wali="{{ $item->nama_wali }}"
                                    data-alamat="{{ $item->alamat_ortu_wali }}"
                                    data-telepon="{{ $item->telepon_ortu_wali }}"
                                >
                    ...
                    <input type="text" name="nama_ortu_wali" readonly>

                    <label>Alamat Orang Tua/Wali</label>
                    <textarea name="alamat_ortu_wali" readonly></textarea>

                    <label>Telepon Orang Tua/Wali</label>
                    <input type="text" name="telepon_ortu_wali" readonly>
...
$(document).ready(function() {
    ...
    $('#tanggal_kunjungan_ow').val(`${year}-${month}-${day}T${hours}:${minutes}`);
    ...
    $('#id_siswa').on('change', function() {
        ...
        let namaAyah = selected.data('nama-ayah') || '';
        let namaIbu  = selected.data('nama-ibu') || '';
        let namaWali = selected.data('nama-wali') || '';

        let gabungan = '';
        if (namaAyah && namaIbu) gabungan = namaAyah + ' - ' + namaIbu;
        else if (namaAyah) gabungan = namaAyah;
        else if (namaIbu) gabungan = namaIbu;
        else if (namaWali) gabungan = namaWali;

        $('input[name="nama_ortu_wali"]').val(gabungan);
        $('textarea[name="alamat_ortu_wali"]').val(selected.data('alamat') || '');
        $('input[name="telepon_ortu_wali"]').val(selected.data('telepon') || '');
    });
});
</script>
</body>
